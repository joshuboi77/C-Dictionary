...
// Import built-in definitions (embedded from C_Dictionary.md)
const BUILT_IN_DEFINITIONS = require('./built-in-definitions.js');
// Import embedded dictionary content
const C_DICTIONARY = require('./c-dictionary.js');

let analyzer = null;

class CTokenAnalyzer {
    constructor() {
        this.tokenDefinitions = new Map();
        this.diagnostics = vscode.languages.createDiagnosticCollection('c-token-analyzer');
        this.statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    }

    async activate(context) {
        console.log('C Token Analyzer extension is now active!');

        // Load token definitions
        await this.loadTokenDefinitions();

        // Register commands
        const showTokenInfoCommand = vscode.commands.registerCommand('c-token-analyzer.showTokenInfo', () => {
            this.showTokenInfo();
        });

        const analyzeFileCommand = vscode.commands.registerCommand('c-token-analyzer.analyzeFile', () => {
            this.analyzeCurrentFile();
        });

        const showDictionaryCommand = vscode.commands.registerCommand('c-token-analyzer.showDictionary', () => {
            this.showDictionary();
        });

        const showTokenDefinitionCommand = vscode.commands.registerCommand('c-token-analyzer.showTokenDefinition', (token) => {
        ...
        // Update status bar
        this.statusBarItem.text = "$(symbol-keyword) C Tokens";
        this.statusBarItem.tooltip = "C Token Analyzer - Hover over tokens for definitions";
